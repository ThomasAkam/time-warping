...
def align_spikes(trial_times, target_times, spikes, pre_win=0, post_win=0, fs_out=25,
                 smooth_SD='default', plot=False):
    ...
    Arguments:
    trial_times : Array of reference point times for each trial (ms) [n_trials, n_ref_points]
    target_times: Reference point times to warp each trial onto (ms) [n_ref_points]
    spikes:  Array of neuron IDs and spike times [2, n_spikes]
             spikes[0,:] is neuron IDs, spikes [1,:] is spike times (ms).
    pre_win: Time window before first event to include in aligned rates (ms).
    post_win: Time window after last event to include in aligned rates (ms).
    ...
    '''
    if smooth_SD == 'default': smooth_SD = 1000/fs_out
    n_trials = trial_times.shape[0]
    neuron_IDs = np.sort(np.unique(spikes[0,:]))
    n_neurons = len(neuron_IDs)
    t_out = np.arange(target_times[0]-pre_win, target_times[-1]+post_win, 1000/fs_out) # Timepoints of output samples.
    pad_len = smooth_SD*4 
    ...
    trial_times  = np.hstack([trial_times[:,0,None]-pre_win-pad_len, trial_times , trial_times[:,-1,None]+post_win+pad_len])
    # Apply timewarping.
    ...
    for tr in range(n_trials): # Loop over trials.
        trial_spikes = spikes[:, (trial_times[tr,0] < spikes[1,:]) & 
                                 (spikes[1,:] < trial_times[tr,-1])]
        ...
        for i in range(len(target_times)-1): # Loop over intervals.
            interval_mask = ((trial_times[tr,i] < trial_spike_times) &  # Boolean mask indicating which spikes are in interval i.
        ...
        for j, n in enumerate(neuron_IDs): # Loop over neurons.
            if n in trial_spike_IDs:
                ...
                aligned_rates[tr,j,:] = 1000*np.sum(norm.pdf(n_spike_times[None,:]
        ...
        if plot: # Plot trial alignment for neuron most active on trial.
            ...
            ylim = [trial_times[tr,0]-trial_times[tr,1]+pad_len, trial_times[tr,-1]-trial_times[tr,1]-pad_len]
            plt.scatter(np.ones(n_spikes)*t_out[0], trial_spike_times[neuron_mask]-trial_times[tr,1], s=60, marker=1)
            plt.scatter(aligned_sp_times[neuron_mask], np.ones(n_spikes)*ylim[0],
                        s=60*spike_stretch[neuron_mask], marker=2)
            plt.plot(target_times,trial_times[tr,:]-trial_times[tr,1],'.:', c='k')
            plt.ylabel('True time (ms)')
            ...
            if input("Press enter for next trial, 'x' to stop plotting:") == 'x':
                plot = False
    return {'aligned_rates'   : aligned_rates,
            't_out'           : t_out,
...
def align_signals(signals, sample_times, trial_times, target_times, pre_win=0, post_win=0,
                  fs_out=25, smooth_SD='auto', plot_warp=False):
    '''
    ...
    Arguments: 
    signals      : Signals to be aligned [n_signals, n_samples]
    sample_times : Times when the samples occured (ms) [n_samples]
    ...
    pre_win      : Time window before first event to include in aligned signals (ms).
    post_win     : Time window after last event to include in aligned signals (ms).
    fs_out       : The sampling rate of the aligned output signals (Hz).
    smooth_SD    : Standard deviation (ms) of Gaussian smoothing applied to output signals.
   ...
   '''
    ...
    n_trials, n_signals, n_timepoints = (trial_times.shape[0], signals.shape[0], len(t_out))
    aligned_signals = np.full([n_trials, n_signals, n_timepoints], np.nan)
    
    for i in np.arange(n_trials):
        if trial_times[i, 0] < sample_times[0]:
            continue # This trial occured before signals started. 
        if trial_times[i,-1] > sample_times[-1]:
        ...
        trial_samples = ((trial_times[i,0] <= sample_times) & (sample_times < trial_times[i,-1]))
        trial_signals = signals[:,np.where(trial_samples)[0]]
        t0 = sample_times[trial_samples]  # Trial sample times before warping
        t1 = np.zeros(len(t0))          # Trial sample times after warping
        for j in range(target_deltas.shape[0]):
            mask = (trial_times[i,j] <= t0) & (t0 < trial_times[i,j+1])
            t1[mask] = (t0[mask]-trial_times[i,j])*target_deltas[j]/trial_deltas[i,j] + target_times[j]

        # Calculate aligned signals using analytical solution to overlap integral between
        # linearly interpolated signals and gaussian around output timepoints.
        aligned_signals[i,:,:] = (np.sum(_int_norm_lin_prod(trial_signals[:,:-1],
            trial_signals[:,1:],t1[:-1],t1[1:],t_out[:,None,None],s=smooth_SD),2).T)
        ...
        if plot_warp: # Plot input and output signals for the most active neurons.
            ...
            plt.plot(t0, trial_signals[most_active,:].T,'.-')
            for x in trial_times[i,:]:
                plt.axvline(x, color='k', linestyle=':')
            plt.xlim(trial_times[i,1]-pre_win, trial_times[i,-2]+post_win)
            plt.ylabel('Activity')
            plt.xlabel('True trial time (ms)')
            plt.subplot2grid((2,3),(1,1), colspan=2)
            plt.plot(t_out,aligned_signals[i,most_active,:].T,'.-')
    ...
    return {'aligned_signals' : aligned_signals, 
            't_out'           : t_out,           
            'min_max_stretch' : min_max_stretch}
